...
import { createDonor, createPatient, getAllDonors, getAllPatients, getDonor, getPatient, } from "./chaincode";
...
app.post("/api/add-patient", async (req: Request, res: Response) => {
  ...
  const { age, height, bloodType, pediatricStatus, location, meldScore, hlaB27Antibodies } = req.body;
  const { client, gateway, network, contract } = await connectToGateway();
  try {
    await createPatient(contract, patientId, age, height, bloodType, pediatricStatus, location, meldScore, hlaB27Antibodies);
  } finally {
    gateway.close();
    client.close();
  }
...
app.post("/api/add-donor", async (req: Request, res: Response) => {
  ...
  const { organ, age, height, bloodType, pediatricStatus, location, hlaB27AntigenTest } = req.body;
  const { client, gateway, network, contract } = await connectToGateway();
  try {
    await createDonor(contract, donationId, organ, age, height, bloodType, pediatricStatus, location, hlaB27AntigenTest);
  } finally {
    gateway.close();
    client.close();
  }
